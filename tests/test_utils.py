...
def test_find_existing_file(
    temp_dir: str, suffix_list: List[str], test_files: List[str]
):
    ...
    for file_name in test_files:
        ...
        found_file = find_existing_file(
            input_file_with_diff_extension, ",".join(suffix_list)
        )
        ...
        assert (
            find_existing_file(non_existent_file, ",".join(suffix_list))
            == non_existent_file
        )
